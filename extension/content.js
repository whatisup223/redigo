  ...
  container.innerHTML = `
    ...
    <div class="redigo-body">
      ...
      <div id="redigo-auto-track-msg" style="font-size: 11px; color:#10b981; text-align:center; font-weight:700; background: #ecfdf5; padding: 10px; border-radius: 12px; border: 1px solid #d1fae5;">
         ✓ Auto-tracking enabled. Just post on Reddit!
      </div>
    </div>
  `;
...
// Listen for auto-confirm from background script webRequest listener
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === 'POST_AUTO_CONFIRMED') {
    const msgEl = document.getElementById('redigo-auto-track-msg');
    if (msgEl) {
      msgEl.innerHTML = '✨ Post successfully tracked in Analytics!';
