...
// Auto-Confirm via JSON request interception
chrome.webRequest.onCompleted.addListener((details) => {
    if (details.method === 'POST') {
        const url = details.url.toLowerCase();
        // Detect Reddit post/comment JSON submissions
        if (url.includes('api/submit') || url.includes('api/comment') || url.includes('graphql')) {
            chrome.storage.local.get(['redigo_assistant_draft', 'redigo_user_id', 'redigo_token'], (res) => {
                if (res.redigo_assistant_draft && res.redigo_user_id) {
                    const payload = {
                    ...
                    // Optional: Broadcast successful deployment back to content script
