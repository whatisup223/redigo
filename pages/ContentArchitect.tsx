                                ...
                                <div className="space-y-4">
                                    <div className="p-5 bg-slate-50 rounded-2xl space-y-3">
                                        ...
                                        <div className="flex items-center gap-3 bg-white p-4 rounded-xl border border-slate-100">
                                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white font-black transition-all ${(extensionDetected === true || (window.innerWidth <= 768)) ? 'bg-emerald-600 shadow-lg shadow-emerald-100' : 'bg-slate-300'}`}>
                                                {(extensionDetected === true || (window.innerWidth <= 768)) ? <Check size={20} /> : extensionDetected === null ? <RefreshCw size={18} className="animate-spin" /> : <AlertCircle size={20} />}
                                            </div>
                                            <div className="flex-1">
                                                <p className="text-sm font-bold text-slate-900">
                                                    {window.innerWidth <= 768 ? 'Mobile Assistant Ready' : extensionDetected === null ? 'Verifying Engine...' : extensionDetected === true ? 'Extension Verified' : 'Extension Missing'}
                                                </p>
                                                <p className="text-[10px] text-slate-500 font-medium">
                                                    {window.innerWidth <= 768 ? 'Manual posting supported via Assistant' : extensionDetected === true ? 'Using active browser identity' : 'Install extension for direct posting'}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
