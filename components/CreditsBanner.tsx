...
const CreditsBanner: React.FC<CreditsBannerProps> = ({ plan, usedCredits, limit }) => {
    const isLimitReached = plan === 'Free' && usedCredits >= limit;
    const percentage = Math.min((usedCredits / limit) * 100, 100);
    ...
    return (
        <div className="bg-slate-900 rounded-[2.5rem] p-6 lg:p-8 flex flex-col md:flex-row items-center justify-between text-white shadow-xl shadow-slate-200 relative overflow-hidden group">
            ...
            <div className="relative z-10 flex items-center gap-5">
                ...
                <div>
                    ...
                    <p className="text-slate-400 text-sm font-medium">
                        You have used <span className="text-white font-black">{usedCredits}/{limit}</span> AI credits today.
                    </p>
                </div>
            </div>
            ...
            <div className="relative z-10 mt-6 md:mt-0 w-full md:w-auto flex items-center gap-6">
                {plan === 'Free' && (
                    <div className="hidden lg:block w-48 h-2 bg-slate-800 rounded-full overflow-hidden">
                        <div
                            className={`h-full transition-all duration-1000 ease-out ${percentage > 80 ? 'bg-red-500' : 'bg-orange-500'}`}
